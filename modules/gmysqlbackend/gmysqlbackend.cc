...
public:
  ...
  void declareArguments(const string &suffix="")
  {
    ...
    declare(suffix,"get-order-first-query","DNSSEC Ordering Query, first", "select ordername, name from records where (auth=1 or (%d=0 and type='NS')) and domain_id=%d order by 1 asc limit 1");
    declare(suffix,"get-order-before-query","DNSSEC Ordering Query, before", "select ordername, name from records where ordername <= '%s' and (auth=1 or (%d=0 and type='NS')) and domain_id=%d order by 1 desc limit 1");
    declare(suffix,"get-order-after-query","DNSSEC Ordering Query, after", "select min(ordername) from records where ordername > '%s' and (auth=1 or (%d=0 and type='NS')) and domain_id=%d");
    declare(suffix,"get-order-last-query","DNSSEC Ordering Query, last", "select ordername, name from records where ordername != '' and (auth=1 or (%d=0 and type='NS')) and domain_id=%d order by 1 desc limit 1");
    ...
    declare(suffix,"set-type-auth-query", "DNSSEC set per-type auth query", "update records set auth=%d where type='%s' and domain_id='%d'");
